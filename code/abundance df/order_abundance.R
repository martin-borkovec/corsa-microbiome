...
saveRDS(seqtab.order, "results/seqtab.order.RDS")

seqtab.pseudocounts_order  <- zCompositions::cmultRepl(seqtab.order, method="CZM", output = "p-counts")

seqtab.clr_order <- log(seqtab.pseudocounts_order / apply(seqtab.pseudocounts_order, 1, psych::geometric.mean))

saveRDS(seqtab.clr_order, "results/seqtab.clr_order.RDS")

clr_long_order <- seqtab.clr_order %>%
  as.data.frame() %>% 
  tibble::rownames_to_column("sampleID") %>% 
  pivot_longer(-sampleID, names_to = "Order", values_to = "clr")

df_order <- seqtab.order %>% 
  as.data.frame() %>% 
  tibble::rownames_to_column("sampleID") %>% 
  pivot_longer(-sampleID, names_to = "Order", values_to = "abundance") %>% 
  left_join(clr_long_order, by = c("sampleID", "Order")) %>% 
  ...
  group_by(Order) %>% 
  mutate(Order_reads = sum(abundance)) %>% 
  mutate(Order_reads_median = median(abundance)) %>% 
  mutate(Order_prevalence = mean(abundance > 0)) %>% 
  group_by(patID, Order) %>% 
...
saveRDS(df_order, "results/df_order.RDS")
