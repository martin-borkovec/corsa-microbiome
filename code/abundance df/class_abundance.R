library(dplyr)
library(tidyr)

df_asv <- readRDS("results/df_asv.RDS")
meta_data <- readRDS("data/2021-08-04 metadata.RDS")

seqtab.class <- 
  df_asv %>% 
  filter(!is.na(Class)) %>% 
  group_by(sampleID, Class) %>% 
  summarise(abundance = sum(abundance)) %>% 
  pivot_wider(names_from = Class, values_from = abundance) %>% 
  tibble::column_to_rownames("sampleID")

saveRDS(seqtab.class, "results/seqtab.class.RDS")

seqtab.pseudocounts_class  <- zCompositions::cmultRepl(seqtab.class, method="CZM", output = "p-counts")

seqtab.clr_class <- log(seqtab.pseudocounts_class / apply(seqtab.pseudocounts_class, 1, psych::geometric.mean))

saveRDS(seqtab.clr_class, "results/seqtab.clr_class.RDS")

clr_long_class <- seqtab.clr_class %>%
  as.data.frame() %>% 
  tibble::rownames_to_column("sampleID") %>% 
  pivot_longer(-sampleID, names_to = "Class", values_to = "clr")

df_class <- seqtab.class %>% 
  as.data.frame() %>% 
  tibble::rownames_to_column("sampleID") %>% 
  pivot_longer(-sampleID, names_to = "Class", values_to = "abundance") %>% 
  left_join(clr_long_class, by = c("sampleID", "Class")) %>% 
  mutate(LNR = substring(sampleID, 1, 5),
         patID = case_when(substring(LNR, 1, 4) == "V001" ~ "v0010",
                           substring(LNR, 1, 4) == "V002" ~ "v0020",
                           substring(LNR, 1, 4) == "V003" ~ "v0030",
                           substring(LNR, 1, 4) == "V004" ~ "v0040",
                           substring(LNR, 1, 4) == "V005" ~ "v0050",
                           TRUE ~ LNR),
         # prevalence = colSums(seqtab.final),
         volunteer = grepl("V", LNR), 
         method = case_when(grepl("FIT", sampleID) ~ "FIT",
                            grepl("Nor", sampleID) ~ "Norgen")) %>% 
  left_join(select(meta_data, patID = LNR, BMI, Alter, Geschlecht), by = "patID") %>% 
  group_by(sampleID) %>% 
  mutate(sample_reads = sum(abundance)) %>% 
  mutate(sample_diversity = sum(abundance > 0)) %>% 
  group_by(patID) %>% 
  mutate(pat_reads = sum(abundance)) %>% 
  group_by(Class) %>% 
  mutate(Class_reads = sum(abundance)) %>% 
  mutate(Class_reads_median = median(abundance)) %>% 
  mutate(Class_prevalence = mean(abundance > 0)) %>% 
  group_by(patID, Class) %>% 
  mutate(matching = sum(abundance > 0) %in% c(0, 2, 6)) %>% 
  mutate(pat_presence = any(abundance > 0)) %>% 
  ungroup %>% 
  mutate(ratio = abundance/sample_reads,
         logratio = log(abundance/sample_reads))
  

saveRDS(df_class, "results/df_class.RDS")

